(function() { # can validate reference iterations

    expect_error(STITCH:::validate_reference_iterations("wer"))
    expect_error(STITCH:::validate_reference_iterations("two"))
    expect_error(STITCH:::validate_reference_iterations(NULL))
    expect_error(STITCH:::validate_reference_iterations(NA))
    expect_error(STITCH:::validate_reference_iterations(0.1))
    expect_error(STITCH:::validate_reference_iterations(11.2))

    expect_null(STITCH:::validate_reference_iterations(0))
    expect_null(STITCH:::validate_reference_iterations(1))
    expect_null(STITCH:::validate_reference_iterations(10))
})()


(function() { # reference position SNPs can be valid for niterations = 1
    expect_equal(
        STITCH:::validate_pos_and_legend_snps_for_niterations_equals_1(
...
(function() { # reference position SNPs can be invalid for niterations = 1
    expect_error(
        STITCH:::validate_pos_and_legend_snps_for_niterations_equals_1(
...
(function() { # reference position SNPs are ignored for niterations > 1
    expect_equal(
        STITCH:::validate_pos_and_legend_snps_for_niterations_equals_1(
...
(function() { # sensible error message thrown when asked to load reference out of range
    ...
    STITCH:::validate_legend_header(legend_header)

    ## no error for >=1 variant loaded
    out <- STITCH:::load_reference_legend(
...
(function() { # reference sample file requires POP column
...
(function() { # reference data can be loaded for an autosome
...
(function() { # haps matrix without NA is passed through
    haps <- array(c(0, 1, 0, 1, 0, 1), c(3, 2))
    expect_equal(
        STITCH:::remove_NA_columns_from_haps(haps),
        haps
    )
})()


(function() { # haps matrix with bad number of NAs is caught
    haps <- array(c(0, 1, 0, 1, 0, 1), c(3, 2))
    haps[2, 2] <- NA
    expect_error(
        STITCH:::remove_NA_columns_from_haps(haps),
...
(function() { # haps matrix all NA is caught
    haps <- array(c(0, 1, 0, 1, 0, 1), c(3, 2))
    haps[, 1] <- NA
    haps[, 2] <- NA
    expect_error(
...
(function() { # haps matrix with one column NA is OK and that column gets removed
    haps <- array(c(0, 1, 0, 1, 0, 1), c(3, 2))
    haps[, 2] <- NA
    expect_equal(
        STITCH:::remove_NA_columns_from_haps(haps),
...
(function() { # reference data can be loaded for chromosome X
...
(function() { # reference data can be loaded for an autosome specifying regionStart, regionEnd and buffer
...
(function() { # reference data can be loaded for chromosome X specifying regionStart, regionEnd and buffer
...
(function() { # reference data can be loaded for a population
...
(function() { # can make fake reference samples in C++
    ...
    iSample <- 1
    sampleReads1 <- STITCH:::make_sampleReads_from_hap(
        non_NA_cols,
        reference_phred,
...
(function() { # can convert a reference hap into eMatGrid_t directly
...
(function() { # can do single reference iteration
...
(function() { # can make reference ematgrid with large grid
