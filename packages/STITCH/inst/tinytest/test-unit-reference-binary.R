(function() { # can properly understand binary integer storage in R
    ...
    data.frame(t(sapply(1:length(to_change), function(i) {
        ...
        x[] <- 0L
        x[to_change[[i]]] <- 1L
        c(STITCH:::int_contract(x), paste0(x, collapse = ""))
    })))
    ...
    for(i in 1:length(to_change)) {
        ...
        expect_equal(STITCH:::int_contract(x), STITCH:::int_contract_manual(x))

        y <- 1L - x
        STITCH:::int_contract_manual(y)
        expect_equal(STITCH:::int_contract(y), STITCH:::int_contract_manual(y))
...
(function() { # can re-order vector of symbols into reverse sorted prefix order
    ...
    rownames(out) <- apply(out, 1, STITCH:::int_contract)
...
(function() { # binary/integer storage format for reference haps makes sense
    ...
    for(nSNPs in c(90, 95, 96, 97)) {
        ...
        for(k in 1:K) {
            expect_equal(rhi_t[k, ], int_expand(STITCH:::int_contract(rhi_t[k, ]), nSNPs))
            expect_equal(rhi_t[k, ], rcpp_int_expand(STITCH:::int_contract(rhi_t[k, ]), nSNPs))
            expect_equal(STITCH:::int_contract(rhi_t[k, ]), rcpp_int_contract(rhi_t[k, ]))
        }
        ...
        for(i in 1:2) {
            ...
            if (i == 3) { hap[] <- 1L}
            expect_equal(STITCH:::int_contract(hap), rcpp_int_contract(hap))
            expect_equal(hap, int_expand(STITCH:::int_contract(hap), nSNPs))
            expect_equal(hap, rcpp_int_expand(STITCH:::int_contract(hap), nSNPs))
        }
...
(function() { # edge cases for reference haps are OK
    ...
    expect_equal(STITCH:::int_contract(hap), rcpp_int_contract(hap))
    expect_equal(int_expand(STITCH:::int_contract(hap), nSNPs), rcpp_int_expand(rcpp_int_contract(hap), nSNPs))

    ## these tests are less useful! NA is OK

})()

(function() { # can quickly calculate distance between binary haps and haplotype dosage vector
...
(function() { # can quickly subset rhb_t to a new eHaps like container
...
(function() { # math for subsetting for start makes sense
    ...
    ## can recreate
    expect_true((max(abs(h3 - h2))) < (1e-08))
    expect_true((max(abs((t(h2) %*% v %*% d_inv)[, 1] - v[, 1]))) < (1e-08))

    ## can re-calculate first few eigenvalues
    expect_true((max(abs((t(h2[, 1:3]) %*% v[, 1:4] %*% d_inv[1:4, 1:4])[1, ] - v[1, 1:4]))) < (1e-08))

    ## versus first four values for a sample
    expect_true((max(abs((t(h2) %*% v[, 1:4] %*% d_inv[1:4, 1:4])[1, ] - v[1, 1:4]))) < (1e-08))
    ...
    ## and can build!
    ## good
    expect_true((max(abs((t(h2b[, ip]) %*% v[, 1:4] %*% d_inv[1:4, 1:4])[ib, ] - v[ib, 1:4]))) < (1e-08))
...
(function() { # can sample haps to use for eHaps start efficiently with binary format
    ...
    for(i_test in 1:4) {
        ...
        cols_to_use <- STITCH:::sample_haps_to_use(
            rhb = rhb,
