...
(function() { # complicated cigar strings are split properly
...
(function() { # a mixture of complicated and uncomplicated cigar strings are split properly
...
(function() { # a soft clipped read is filtered appropriately when not using soft clipped bases
    splitCigarRead <- list(list(c(3, 3, 4), c("S", "M", "S")))
    out <- STITCH:::deal_with_soft_clipped_bases(
...
(function() { # a soft clipped read is filtered appropriately when with a normal read when not using soft clipped bases
    splitCigarRead <- list(
...
(function() { # a soft clipped read is filtered appropriately when using soft clipped bases
...
(function() { # a soft clipped read is filtered appropriately when with a normal read when using soft clipped bases
    splitCigarRead <- list(
        list(c(11, 12, 13), c("M", "I", "M")),
