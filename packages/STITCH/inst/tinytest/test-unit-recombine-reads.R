(function() { # can split a read
    ...
    set.seed(1)
    out <- STITCH:::get_sampleRead_from_SNP_i_to_SNP_j(
        sampleRead,
        1,
        2,
        L,
...
(function() { # a good read does not want to get split
    ...
    w <- STITCH:::get_reads_worse_than_50_50(
...
(function() { # a bad read wants to get split
...
(function() { # correctly split a bad read
    ...
    out <- STITCH:::get_default_hapProbs(pseudoHaploidModel = 9, sampleReads = sampleReads, S = S)
    ...
    tempdir <- tempdir()
    out <- STITCH:::findRecombinedReadsPerSample(
        gammaK_t = gammaK_t,
        eHapsCurrent_t = eHapsCurrent_t,
        K = K,
        L = L,
    ...
    expect_equal(
        sampleReads[[2]],
        STITCH:::get_sampleRead_from_SNP_i_to_SNP_j(
            sampleRead, 1, 2, L, grid
        )
    )

    ## do not bother setting seed - is second sample call
    new_read_2 <- STITCH:::get_sampleRead_from_SNP_i_to_SNP_j(
        sampleRead, 3, 4, L, grid
    )

    load(file = STITCH:::file_sampleProbs(tempdir, iSample, regionName))
...
(function() { # correctly split a bad read with grid mode
