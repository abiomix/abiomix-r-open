(function() { # reference legend file with duplicate entries throws an error
    ...
    expect_error(
        STITCH:::validate_reference_legend(reference_legend, "dummy.txt"),
...
(function() { # dependency checker works
...
(function() { # an error is thrown when writing to an unwrittable directory
...
(function() { # an error is thown when bamlist does not exist
...
(function() { # an error is thown when cramlist does not exist
...
(function() { # can print allele count properly for empty results
    alleleCount <- array(0, c(10, 3))
    alleleCount[, c(1, 3)] <- NA
    STITCH:::print_allele_count(alleleCount, N = 10)
})()

(function() { # can print allele count properly for normal results
    set.seed(1)
...
(function() { # can print allele count for variable results
...
(function() { # can validate method
    expect_equal(STITCH:::validate_method("diploid"), NULL)
    expect_equal(STITCH:::validate_method("pseudoHaploid"), NULL)
    expect_equal(STITCH:::validate_method("diploid-inbred"), NULL)
    expect_error(STITCH:::validate_method("haploid"))
})()

(function() { # can validate K

    expect_null(STITCH:::validate_K(1))
    expect_null(STITCH:::validate_K(10))
    expect_null(STITCH:::validate_K(100))

    expect_error(STITCH:::validate_K(0))
    expect_error(STITCH:::validate_K(0.5))
    expect_error(STITCH:::validate_K(4.2))
    expect_error(STITCH:::validate_K(-10))
    expect_error(STITCH:::validate_K("10"))
    expect_error(STITCH:::validate_K("word"))
