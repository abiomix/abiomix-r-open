(function() { # can determine how assign SNPs to blocks for output on toy example

    grid <- 0:9 ## 0-based
    start_and_end_minus_buffer <- c(1, 10) ## 1-based
    outputSNPBlockSize <- 1000
    blocks_for_output <- STITCH:::determine_snp_and_grid_blocks_for_output(
        grid = grid,
...
(function() { # can determine how assign SNPs to blocks for output on tiny example
...
(function() { # can determine how assign SNPs to blocks for output on another toy example
...
(function() { # can determine how assign SNPs to blocks for tiny outputSNPBlockSize
...
(function() { # can determine how assign SNPs to blocks for output on yet another toy example
...
(function() { # can determine how assign SNPs to blocks for output
...
(function() { # can properly assign reads to those blocks in which they are needed
    ...
    for(gridWindowSize in c(NA, 15)) {
        ...
        blocks_in_vector_form <- STITCH:::get_blocks_in_vector_form(blocks_for_output, nGrids)

        ## make fake sampleReads with reads everywhere!
        sampleReads <- f(grid)
        out <- STITCH:::determine_starts_ends_whats_for_sampleReads(sampleReads, blocks_in_vector_form)
