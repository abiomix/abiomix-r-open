(function() { # can do pseudoHaploid updates in C++
    ...
    ## R version
    outR <- STITCH:::R_pseudoHaploid_update_9(
...
(function() { # forwardBackwardDiploid and forwardBackwardHaploid work
    ...
    for(gridWindowSize in c(NA, 3)) {
        ...
        fbsoL <- STITCH:::run_forward_backwards(
...
(function() { # can sample one path from forwardBackwardDiploid
    ...
    ## )
    set.seed(50)
    out <- STITCH:::run_forward_backwards(
        sampleReads = sampleReads,
        priorCurrent_m = priorCurrent_m,
        transMatRate_tc_D = transMatRate_tc_D,
        alphaMatCurrent_tc = alphaMatCurrent_tc,
    ...
    out1 <- STITCH:::run_forward_backwards(
        sampleReads = sampleReads,
        priorCurrent_m = priorCurrent_m,
        transMatRate_tc_H = transMatRate_tc_H,
        alphaMatCurrent_tc = alphaMatCurrent_tc,
...
(function() { # can calculate eMatHapSNP and sample a haploid path
