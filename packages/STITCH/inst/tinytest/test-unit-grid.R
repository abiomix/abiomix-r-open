...
(function() { # can validate gridWindowSize
    expect_null(STITCH:::validate_gridWindowSize(as.numeric(1000)))
    expect_null(STITCH:::validate_gridWindowSize(as.integer(1000)))
    expect_null(STITCH:::validate_gridWindowSize(10))
    expect_null(STITCH:::validate_gridWindowSize(3))
    expect_null(STITCH:::validate_gridWindowSize(NA))
    expect_error(STITCH:::validate_gridWindowSize(1))
    expect_error(STITCH:::validate_gridWindowSize(10.5))
    expect_error(STITCH:::validate_gridWindowSize("1000"))
})()

(function() { # can assign physical positions to grid
...
(function() { # distances on the grid are OK
...
(function() { # removal of buffer from grid makes sense
...
## Deprecated grid/GP scratch test:
## this is deprecated, and used in test-unit-cpp


(function() { # can downsample for gridding appropriately
    ...
    for(downsampleToCov in c(1, 2)) {
        ...
        out <- STITCH:::downsample_snapped_sampleReads(
            sampleReads,
            iBam,
            downsampleToCov,
            sampleNames,
