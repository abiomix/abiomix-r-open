...
(function() { # can understand genetic map format
    ...
    expect_equal(
        STITCH:::fill_in_genetic_map_cm_column(genetic_map)[, 3],
        genetic_map[, 3],
        tolerance = 1e-7
    )
    expect_equal(
        STITCH:::fill_in_genetic_map_rate_column(genetic_map)[, 2],
        genetic_map[, 2]
    )
...
(function() { # can understand genetic map format second file
    ...
    expect_equal(
        max(STITCH:::fill_in_genetic_map_rate_column(genetic_map)[, 2] -         genetic_map[, 2]) < 1e-4,
        TRUE
    )
...
(function() { # can load and validate reference genetic map
    ...
    expect_null(
        STITCH:::validate_genetic_map(genetic_map)
    )

})()

(function() { # can error invalid genetic reference map

    L <- 1:10
    n_snps <- 10
    genetic_map <- make_genetic_map_file(L, n_snps, expRate = 0.5)
    genetic_map[5, "Genetic_Map.cM."] <- 2 * genetic_map[5, "Genetic_Map.cM."]
    expect_error(STITCH:::validate_genetic_map(genetic_map, verbose = FALSE))

})()


(function() { # can simply match genetic map to desired positions
    ...
    genetic_map <- STITCH:::fill_in_genetic_map_rate_column(genetic_map)
    ...
    truth_genetic_map <- STITCH:::fill_in_genetic_map_rate_column(truth_genetic_map)

    difference <- truth_genetic_map[, "Genetic_Map.cM."] - cM
    expect_true((max(difference)) < (1e-04))

})()


(function() { # can initialize sigmaCurrent the same way with constant rate
    ...
    original <- STITCH:::initialize_sigmaCurrent_m(cM_grid = NULL, nGen = nGen, nGrids = nGrids, S = S, dl = dl, expRate = expRate)
    new <- STITCH:::initialize_sigmaCurrent_m(cM_grid = cM_grid, nGen = nGen, nGrids = nGrids, S = S, dl = dl, expRate = expRate)
...
(function() { # can assign grid positions when genetic map is given
    ...
    expect_true((length(unique(out$grid[2501 <= L & L <= 3501]))/1000) > (out$nGrids/10000))
    expect_true((length(unique(out$grid[5001 <= L & L <= 7000]))/1000) > (out$nGrids/10000))

    smoothed_cM <- STITCH:::make_smoothed_cM_rate(
        cM = cM,
        L_grid = L_grid,
...
(function() { # can make genetic map file from VCF
    ...
    expect_null(STITCH:::validate_genetic_map(genetic_map, verbose = FALSE))
...
(function() { # can make genetic map file from VCF without specifying region
