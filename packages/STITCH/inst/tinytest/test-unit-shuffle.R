(function() { # can determine switches
    ...
    switchOrder <- STITCH:::determine_switch_order(fromMat, nbreaks, K)
...
(function() { # can run getBetterSwitchesSimple with / without grid
    ...
    for(gridWindowSize in c(NA, 500)) {
        ...
        out <- STITCH:::getBetterSwitchesSimple(
            fromMat = fromMat,
            nbreak = nbreak,
...
(function() { # can smooth simple rate in R
    ...
    ## now - smooth the rate
    results1 <- STITCH:::make_smoothed_rate(
...
(function() { # can define breaks on tiny region
    ...
    for(gridWindowSize in c(NA, 10000)) {
        ...
        for(i in 1:2) {
            ...
            STITCH:::define_and_save_breaks_to_consider(
                tempdir = tempdir,
            ...
            load(STITCH:::file_break_results(tempdir, regionName))
            ## have an output
...
(function() { # can choose points to break if highest on the left
    ...
    out <- STITCH:::choose_points_to_break(
        smoothed_rate,
        nGrids,
        L_grid,
        shuffle_bin_radius,
...
(function() { # can choose points to break if highest on the right
...
(function() { # can nuke SNPs, accounting for differences in grid distances
    ...
    for(x in c(1, 4)) {
        ...
        for(i_where in 1:3) {
            ...
            out <- STITCH:::get_snps_to_nuke(grid_distances, nGrids, shuffle_bin_radius, snp_best, snp_left, snp_right)
...
(function() { # can smooth rate with lots of small peaks
...
(function() { # can smooth rate with lots of big peaks

    ##w <- L_grid >= 2000000 & L_grid <= 3000000
    ##smoothed_rate <- smoothed_rate[w]
    ##L_grid <- L_grid[w]
    ## save(smoothed_rate, L_grid, file = "STITCH/inst/tinytest/data-shuffle.RData")
...
(function() { # can use determine where to stop in cpp
