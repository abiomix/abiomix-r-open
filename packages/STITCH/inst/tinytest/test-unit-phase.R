(function() { # phase estimation can work with no errors
    test <- cbind(
        c(0, 1, 0, 0),
        c(1, 0, 1, 1)
    )
...
(function() { # phase estimation can work with a het wrongly assigned
    test <- cbind(
        c(0, 1, 0, 0),
        c(1, 0, 1, 1)
    )
...
(function() { # phase estimation skips truth hom sites
    test <- cbind(
        c(0, 0, 0, 0, 0),
        c(1, 1, 0, 1, 1)
    )
...
(function() { # phase estimation deals with non-integer test data
    test <- cbind(
...
(function() { # can proportion estimated haplotype counts
    ...
    test <- STITCH:::estimate_read_proportions(
        sampleReads,
        pRgivenH1,
        pRgivenH2,
        T
...
(function() { # can write out VCF column with expected read counts
    ...
    test1 <- STITCH:::make_column_of_vcf(gp_t, read_proportions)
...
(function() { # can do method 3 phasing in R and Rcpp
    ...
    out1 <- STITCH:::phase_sample_paths_method_3(
    ...
    out2 <- STITCH:::rcpp_phase_sample_paths_method_3(
        alphaMatCurrent_tc = alphaMatCurrent_tc,
        transMatRate_tc_H = transMatRate_tc_H,
        alphaHat_t = alphaHat_t,
        betaHat_t = betaHat_t,
