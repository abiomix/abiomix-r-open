(function() { # can run forwardBackward, then re-run using list of forward and backward probabilities
        ...
        for(output_haplotype_dosages in c(FALSE, TRUE)) {
            ...
            for (method in c("diploid", "pseudoHaploid", "diploid-inbred")) {
                ...
                if (method == "pseudoHaploid") {
                    out <- STITCH:::get_default_hapProbs(
                ...
                fbsoL1 <- STITCH:::run_forward_backwards(
                    sampleReads = sampleReads,
                ...
                gp_t_all <- STITCH:::calculate_gp_t_from_fbsoL(
                    fbsoL = fbsoL1,
                    method = method
                )
                ...
                rse <- STITCH:::determine_starts_ends_whats_for_sampleReads(
                    sampleReads = sampleReads,
                ...
                for(i_output_block in 1:nrow(blocks_for_output)) {
                    ...
                    list_of_alphaBetaBlocks <- lapply(fbsoL1, function(x) x[["list_of_alphaBetaBlocks"]])
                    fbsoL2 <- STITCH:::run_forward_backwards(
                    ...
                    gp_t_local <- STITCH:::calculate_gp_t_from_fbsoL(
                        fbsoL = fbsoL2,
                        method = method
                    )
